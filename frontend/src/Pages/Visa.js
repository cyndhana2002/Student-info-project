...
class Visa extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/visa/generatevisa', value, { headers, withCredentials: true, responseType: 'arraybuffer' });
      ...
      const fontSize = 14;
      const heading = `F.No : DEAN/CEG/BF/BKLN/${this.state.fno}/158`;
      const date = `Dated:`;
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <Row>
            <Col className='annaimagecolvisa'>
        <img src={anna}  className="annaimag"/>
        </Col><Col className='annaformatvisa'>
          <br/>
...
<Card className="contenttypevisa">
<h3 className="contentwritevisa">Please type here your letter with the format:</h3>
<textarea className="inputfield"  value={this.state.name}
                onChange={this.handleNameChange} placeholder='This certificate is issued to enable the student to'></textarea>
<br/>
<div>
...
export default withAuthentication(Visa);